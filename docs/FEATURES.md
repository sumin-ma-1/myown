...
<p align="center"><em>Phase 1~2 구현 · Phase 3~4 계획</em></p>

> **서비스명:** MyOwn  
> **한 줄 요약:** 개인 일정·업무·첨부 문서를 기억하고, 텔레그램으로 알려주며, 완료 시 목록에서 제거하는 업무 관리 AI  
> **UI:** Telegram (확장 예정)
> **대상:** 1인 개인 사용 (화이트리스트 기반)
...
| **알림** | 마감 임박·당일·사용자 지정 시각에 텔레그램으로 통지 |
...
| | 일반 리마인더 앱 | MyOwn |
|--|------------------|-------|
| 입력 | 수동 입력 위주 | 자연어 + 문서 첨부 |
| 문서 | 미지원 | HWP/PDF 등 분석 |
| AI | 없음 | 도구 호출 기반 에이전트 |
| UI | 전용 앱 | 이미 쓰는 Telegram |
...
| 인라인 버튼 | 완료 / 1시간 후 / 상세 | ✅ |
| 명령어 | `/start`, `/list`, `/today`, `/add`, `/done`, `/remind` | ✅ |
...
```
users
  ├─ id, telegram_user_id, timezone, preferences
  │
attachments
...
| 항목 | 방식 |
|------|------|
| 사용자 인증 | Telegram `user_id` 화이트리스트 |
| 거부 시 안내 | 본인 ID 표시 → `.env` 수정 안내 |
...
[Telegram] ←→ [grammY Gateway]
        ...
        ┌───────────┼───────────┬──────────────┐
        ▼           ▼           ▼              ▼
   [Agent Runtime] [Task Service] [Attachment] [Reminder Worker]
        │           │           │              │
        ▼           ▼           ▼              ▼
   [LLM API]   [PostgreSQL]  [hwp-parser]  [Redis/BullMQ]
   OpenAI or       │          sidecar
   Ollama tunnel   └─ attachments, tasks, reminders
```
...
| 그룹 채팅 | 개인 DM 전용 설계. |
| 멀티 사용자 | 화이트리스트는 복수 ID 가능, 멀티테넌트 아님. |
