...
function snapTimeFromOffset(hour: number, offsetY: number): string {
  const minute = offsetY > HOUR_PX / 2 ? 30 : 0;
...
export function DayTimeline({
  ...
  return (
    ...
    >
      ...
      <ScrollFadeArea
        hideScrollbar
        wrapperClassName="min-h-0 flex-1 rounded-lg border border-slate-200 dark:border-slate-700"
        className="h-full max-h-[42rem] lg:max-h-none"
      >
        <div className="relative" style={{ height: GRID_HEIGHT }}>
          {HOURS.map((hour) => (
            <button
              key={hour}
              type="button"
              className="absolute right-0 left-0 flex border-t border-slate-100 text-left hover:bg-slate-50/80 dark:border-slate-800 dark:hover:bg-white/[0.03]"
              style={{ top: hour * HOUR_PX, height: HOUR_PX }}
              onClick={(event) => {
                const rect = event.currentTarget.getBoundingClientRect();
                onSlotClick?.(dayKey, snapTimeFromOffset(hour, event.clientY - rect.top));
              }}
            >
          ...
          <div className="pointer-events-none absolute inset-y-0 left-12 right-1">
          ...
          {isToday && nowMin >= 0 && nowMin <= MINUTES_IN_DAY && (
            <div
              className="pointer-events-none absolute right-0 left-12 z-[2] border-t border-red-500"
